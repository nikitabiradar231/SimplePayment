...
export const STELLAR_EXPERT_TESTNET_URL = "https://stellar.expert/explorer/testnet/search?term=";

export function getStellarLabExplorerUrl(hash) {
  if (!hash) return "https://laboratory.stellar.org/#explorer?network=test";
  return `https://laboratory.stellar.org/#explorer?resource=transactions&endpoint=single&values=${encodeURIComponent(JSON.stringify({ transaction_id: hash }))}&network=test`;
}

export function getStellarExpertExplorerUrl(hash) {
  if (!hash) return "https://stellar.expert/explorer/testnet";
